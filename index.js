...
module.exports = alpha = async (alpha, bot) => {
    ...
    try {
        ...
        switch (command) {
            ...
            case 'script':
            case 'scrip': {
                alpha.reply("Código fuente de este bot", {
                    reply_markup: {
                        inline_keyboard: [
                            [{
                                text: 'Github',
          ...
          case 'help': {
              ...
              let dateIslamic = Intl.DateTimeFormat('es' + '-TN-u-ca-islamic', {
                  day: 'numeric',
                  month: 'long',
                  year: 'numeric'
              }).format(dnew)
          ...
          break
            ...
            case "sasuke": {
                reply(lang.wait)
                let imageUrl = `https://api-ash.zipponodes.xyz/api/anime/${command}`
            ...
            break
            //download
            case 'youtube': case 'yt': case 'ytv': case 'mp4': case 'ytmp4': {
            if (!text) return reply(`*Putting URL YouTube*`)
            if (!isUrl(args[0])) return itsuka.reply(`Invalid URL\n\nMungkin kamu salah memasukkan url atau memberikan spasi saat menggunakan fitur ini\n\nUsahakan tidak ada spasi.\nContoh: ${prefix+command} url`)
            if (!args[0].includes('youtu.be') && !args[0].includes('youtube.com')) return 
            reply(lang.wait)
            let res = await fetch(`https://pnggilajacn.my.id/api/download/ytmp4?url=`+args[0])
            let result = await res.json()
            let { id, thumbnail, title, size, download } = result.result
            ...
            case 'yta': case 'mp3': case 'ytmp3': {
                if (!text) return reply(`*Colocando URL de YouTube*`);
                if (!isUrl(args[0])) return itsuka.reply(`URL no válida\n\nEs posible que hayas ingresado incorrectamente la URL o hayas dejado espacios al usar esta función\n\nAsegúrate de que no haya espacios.\nEjemplo: ${prefix+command} url`);
                if (!args[0].includes('youtu.be') && !args[0].includes('youtube.com')) return itsuka.reply(`URL no válida\n\nEs posible que hayas ingresado incorrectamente la URL o hayas dejado espacios al usar esta función\n\nAsegúrate de que no haya espacios.\nEjemplo: ${prefix+command} url`);
                reply(lang.wait);
                let res = await fetch(`https://pnggilajacn.my.id/api/download/ytmp3?url=` + args[0]);
                let result = await res.json();
                let { id, thumbnail, title, size, download } = result.result;
                let key = "「 AUDIO DE YOUTUBE 」\n\n";
                key += `• Id: ${id}\n`;
                key += `• Título: ${title}\n`;
                key += `• Calidad: 128 kbps\n`;
                key += `• Tamaño: ${size}\n\n`;
                key += `Audio en proceso, por favor espera...`;
                if (size > 20000) return reply(`El tamaño del archivo supera el límite, descárgalo tú mismo ☠️\n\nDescargar: https://ytdl.pnggilajacn.my.id/?url=` + id + `&filter=audioonly&quality=highestaudio&contenttype=audio/mpeg`);
                await alpha.replyWithPhoto({
                    url: thumbnail
                }, {
                    caption: key
            ...
            break
            case 'play': {
    if (!text) return reply(`Enviar comando:\n${prefix + command} Título de la canción\n\nEjemplo de uso:\n\`${prefix + command} bot WhatsApp Zeeoneofc\``);

    reply(lang.wait);

    try {
        let res = await fetch(`https://api-ash.zipponodes.xyz/api/ytplay?url=${encodeURIComponent(text)}`);
        ...
        // Accede a las propiedades dentro de 'resultado'
        var {
            channelUrl,
            views,
            category,
        ...
        // Verifica si las variables están definidas antes de usarlas en el mensaje
        id = id || 'No disponible';
        title = title || 'No disponible';
        views = views || 'No disponible';
        seconds = seconds || 'No disponible';
        channel = channel || 'No disponible';
        publicDate = publicDate || 'No disponible';
        url = url || 'No disponible';

        // Truncar la descripción a 300 caracteres
        description = description ? (description.length > 300 ? description.substring(0, 297) + '...' : description) : 'Sin descripción disponible';
...
Envía el siguiente comando para descargar medios
${prefix}ytmp3 ${url}
${prefix}ytmp4 ${url}`;

        if (!image) {
            // Manejar el caso cuando 'image' es undefined
            // Por ejemplo, puedes enviar un mensaje de error al usuario
            reply('Lo siento, no pude encontrar una imagen para este video.');
            return;
        }

        alpha.replyWithPhoto({ url: image }, {
            caption: thumbInfo,
            parse_mode: 'MARKDOWN',
            reply_markup: {
                inline_keyboard: [
                    [
                        ...
                        {
                            text: 'Video 🎦',
                            callback_data: `${prefix}ytmp4 ${url}`
                        }
                    ]
                ]
            }
...
break     
          /*  case 'play': {
            ...
            if (!res.videos.length) return reply(`Error 404`)
            reply(lang.wait)
            let anu = res.videos[Math.floor(Math.random() * res.videos.length)]
            let { title, thumbnail, videoId, duration, description, views, ago } = anu
            let url = 'https://youtu.be/' + videoId 
            let capt = `*Title:* ${title}\n*Published:* ${ago}\n*Duration:* ${duration}\n*Views:* ${views}\n*Description:* ${description}\n*Url:* ${url}\n\n\nKirim perintah berikut untuk mendownload media\n${prefix}ytmp3 ${url}\n${prefix}ytmp4 ${url}`
            await alpha.replyWithPhoto({
    url: thumbnail
}, {
    caption: capt,
    ...
    reply_markup: {
        inline_keyboard: [
            ...
            case 'yts':
            case 'ytsearch': {
                if (!text) return reply(`Enviar comando:\n${prefix+command} Título de la canción/video\n\nEjemplo de uso:\n${prefix+command} bot WhatsApp Zeeoneofc`)
                reply(lang.wait)
                let res = await fetch(`https://api-ash.zipponodes.xyz/api/ytsearch?text=${encodeURIComponent(text)}`)
                if (!res.ok) throw await res.text()
                var result = await res.json()
                let dapet = result.resultado
                var tbuff = dapet[0].thumbnail
                cap = "「 YOUTUBE SEARCH 」\n\n"
                for (let v = 0; v < 3; v++) {
                    cap += `💬 Titulo : ${dapet[v].title}
            📺 Vistas : ${dapet[v].views}
            ⏰ Duración : ${dapet[v].duration}
            📆 Subido : ${dapet[v].uploaded}
            🔗 URL : ${dapet[v].url}\n\n---------------------------\n\n`
                }
                alpha.replyWithPhoto({
                    url: tbuff
...
case "ig":
case "igdl": {
    if (!args[0]) return reply(`Enviar comando:\n${prefix+command} Enlace de Instagram\n\nEjemplo de uso:\n${prefix+command} https://www.instagram.com/p/ClU74LNpgaw/?igshid=YmMyMTA2M2Y=`)
    if (!isUrl(args[0])) return reply(`Enviar comando:\n${prefix+command} Enlace de Instagram\n\nEjemplo de uso:\n${prefix+command} https://www.instagram.com/p/ClU74LNpgaw/?igshid=YmMyMTA2M2Y=`)
    reply(lang.wait)
    let res = await fetch(`https://api-ash.zipponodes.xyz/api/v1/igdl?url=${encodeURIComponent(args[0])}`)
    if (!res.ok) throw await res.text()
    var result = await res.json()
...
break
            case "pinterest": {
                if (!text) return reply(`Enviar comando:\n${prefix+command} Consulta\n\nEjemplo de uso:\n${prefix+command} sakura`)
                reply(lang.wait)
                let res = await fetch(`${global.APIs.can}/api/search/pinterest?query=${encodeURIComponent(text)}`)
                if (!res.ok) throw await res.text()
                var result = await res.json()
                var images = result.resultado
                // Elimina la primera imagen de la lista
                images.shift()
            ...
            case "mf":
            case "mediafire": {
    if (!args[0]) return reply(`Enviar comando:\n${prefix+command} Enlace de Mediafire\n\nEjemplo de uso:\n${prefix+command} https://www.mediafire.com/file/eb14v8x4oz7ok3h/Alphabot-Mdv17.5-withModule.zip/file`)
    if (!isUrl(args[0]) && !args[0].includes("mediafire.com")) return reply(`Enviar comando:\n${prefix+command} Enlace de Mediafire\n\nEjemplo de uso:\n${prefix+command} https://www.mediafire.com/file/eb14v8x4oz7ok3h/Alphabot-Mdv17.5-withModule.zip/file`)
    reply(lang.wait)
    let res = await fetch(`https://api.akuari.my.id/downloader/mediafireDl?link=${encodeURIComponent(args[0])}`)
    if (!res.ok) throw await res.text()
    var result = await res.json()
    var {
        title: nama,
    ...
    console.log(size)
    if (size.replace('MB', '').trim() >= 100 || size.replace('GB', '').trim() >= 1) { //size edit sendiri jika mau download yang lebih media yang lebih besar
        var key = `「 Mediafire Download 」\n\n`
        key += `Nombre: ${nama}\n`
        key += `Tamaño: ${size}\n`
        key += `Link: ${link}\n\n`
        key += `Para tamaños que superen el límite, por favor descargue a través del enlace de arriba`
        reply(key)
    } else {
        var key = `「 Mediafire Download 」\n\n`
        key += `Nombre: ${nama}\n`
...
break
            case "tiktoknowm":
            case "tiktok": {
                if (!args[0]) return reply(`Enviar comando:\n${prefix+command} Enlace del video de TikTok\n\nEjemplo de uso:\n${prefix+command} https://vm.tiktok.com/ZMjPXawEV`)
                if (!isUrl(args[0]) && !args[0].includes("tiktok.com")) return reply(`Enviar comando:\n${prefix+command} Enlace de video de TikTok\n\nEjemplo de uso:\n${prefix+command} https://vm.tiktok.com/ZMjPXawEV`)
                reply(lang.wait)
                let res = await fetch(`https://api-ash.zipponodes.xyz/api/tiktokv1?url=${encodeURIComponent(args[0])}`)
                if (!res.ok) throw await res.text()
                var result = await res.json()
            ...
            case "zombie3d": {
                ...
                if (!text.includes('|')) return reply(`Enviar comando:\n${prefix+command} Texto\n\nEjemplo de uso:\n${prefix+command} zeeoneofc`)
                reply(lang.wait)
            ...
            break
            case 'banneroflol': {
                if (!text) return reply(`Enviar comando:\n${prefix+command} texto1|texto2|banner\n\nEjemplo de uso:\n${prefix+command} zeeone|ofc|neeko`)
                if (!text.includes('|')) return reply(`Enviar comando:\n${prefix+command} texto1|texto2|banner\n\nEjemplo de uso:\n${prefix+command} zeeone|ofc|neeko`)
                var memek = args.join(" ").split("|")
                var m1 = memek[0]
                var m2 = memek[1]
                var m3 = memek[2]
                if (!m3) return reply(`Enviar comando:\n${prefix+command} texto1|texto2|banner\n\nEjemplo de uso:\n${prefix+command} zeeone|ofc|neeko`)
            ...
            break
            case 'teamlogo': {
                if (!text) return reply(`Enviar comando:\n${prefix+command} texto1|texto2|background\n\nEjemplo de uso:\n${prefix+command} zeeone|ofc|buffalo`)
                if (!text.includes('|')) return reply(`Enviar comando:\n${prefix+command} texto1|texto2|background\n\nEjemplo de uso:\n${prefix+command} zeeone|ofc|buffalo`)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            break
            case 'pencilsketch': {
                if (!text) return reply(`Enviar comando:\n${prefix+command} texto1|texto2|icon\n\nEjemplo de uso:\n${prefix+command} zeeone|ofc|panda`)
                if (!text.includes('|')) return reply(`Enviar comando:\n${prefix+command} texto1|texto2|icon\n\nEjemplo de uso:\n${prefix+command} zeeone|ofc|panda`)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            break
            case 'letterlogos': {
                if (!text) return reply(`Enviar comando:\n${prefix+command} texto1|texto2|thumb\n\nEjemplo de uso:\n${prefix+command} zeeone|ofc|z`)
                if (!text.includes('|')) return reply(`Enviar comando:\n${prefix+command} texto1|texto2|thumb\n\nEjemplo de uso:\n${prefix+command} zeeone|ofc|z`)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
...
${prefix}nuliskiri Subscribe Ya YT zeeoneofc`)
            ...
            case 'nuliskanan':
            case 'nuliskiri': {
                if (text.length < 2) return reply(`Enviar comando:\n${prefix+command} teks\n\nEjemplo de uso\n${prefix+command} ini tulisan saya`)
                reply(lang.wait)
                const tulisan = text
            ...
            case "bot":
            {
                if (!text) return reply(`Enviar comando:\n${prefix+command} Mensaje\n\nEjemplo de uso:\n${prefix+command} Hola, ¿cómo estás?`)
                reply(lang.wait)
                let res = await fetch(`https://api-ash.zipponodes.xyz/api/chatgpt?text=${encodeURIComponent(text)}`)
                if (!res.ok) throw await res.text()
                var result = await res.json()
            ...
            case "ssweb":
            {
                if (!args[0]) return reply(`Enviar comando:\n${prefix+command} URL del sitio web\n\nEjemplo de uso:\n${prefix+command} https://www.example.com`)
                if (!isUrl(args[0])) return reply(`Enviar comando:\n${prefix+command} URL del sitio web\n\nEjemplo de uso:\n${prefix+command} https://www.example.com`)
                reply(lang.wait)
                let imageUrl = `https://api-ash.zipponodes.xyz/api/ssweb?url=${encodeURIComponent(args[0])}`
                alpha.replyWithPhoto({
                    url: imageUrl
                }, {
                    caption: lang.ok
            ...
            break
            case "imagen": {
                if (!text) return reply(`Enviar comando:\n${prefix+command} Texto de búsqueda\n\nEjemplo de uso:\n${prefix+command} gatos`)
                reply(lang.wait)
                let res = await fetch(`https://api-ash.zipponodes.xyz/api/googleImage?text=${encodeURIComponent(text)}`)
                if (!res.ok) throw await res.text()
                var result = await res.json()
          ...
          case "dall-e": {
              if (!text) return reply(`Enviar comando:\n${prefix+command} Texto de búsqueda\n\nEjemplo de uso:\n${prefix+command} gatos`)
              reply(lang.wait)
              let res = await fetch(`https://api.akuari.my.id/ai/dall-e?prompt=${encodeURIComponent(text)}`)
              if (!res.ok) throw await res.text()
          ...
          case "playstore": {
              ...
              reply(lang.wait)
              let res = await fetch(`https://api.akuari.my.id/search/playstoresearch?query=${encodeURIComponent(args[0])}`)
              if (!res.ok) throw await res.text()
              var result = await res.json()
              var apps = result.hasil

              // Siempre envía la primera opción
              var firstApp = apps[0]
              var firstAppDetails = `Título: ${firstApp.title}\nURL: ${firstApp.url}\nDesarrollador: ${firstApp.developer}\nPrecio: ${firstApp.price}\nPuntuación: ${firstApp.scoreText}\n`
              reply(firstAppDetails)

              // Elimina la primera opción de la lista de aplicaciones
              apps.shift()

              // Selecciona cuatro aplicaciones al azar de la lista restante
              for (var i = 0; i < 4; i++) {
                  var randomIndex = Math.floor(Math.random() * apps.length)
                  var randomApp = apps[randomIndex]
                  var randomAppDetails = `Título: ${randomApp.title}\nURL: ${randomApp.url}\nDesarrollador: ${randomApp.developer}\nPrecio: ${randomApp.price}\nPuntuación: ${randomApp.scoreText}\n`
                  reply(randomAppDetails)

                  // Elimina la aplicación seleccionada de la lista de aplicaciones
                  apps.splice(randomIndex, 1)
              }
          }
          break
            case "apk": {
                if (!args[0]) return reply(`Enviar comando:\n${prefix+command} Nombre de la aplicación\n\nEjemplo de uso:\n${prefix+command} WhatsApp`)
                reply(lang.wait)
                let res = await fetch(`https://api.akuari.my.id/downloader/apkdownloader2?package=${encodeURIComponent(args[0])}`)
                if (!res.ok) throw await res.text()
                var result = await res.json()
                var app = result.respon

                // Limita la descripción a 200 caracteres
                var description = app.description.substring(0, 200) + "..."

                var appDetails = `Título: ${app.title}\nDescripción: ${description}\nInstalaciones: ${app.install}\nPuntuación: ${app.score}\nDesarrollador: ${app.developer}\nURL de descarga: ${app.downloadapk}\n`
                reply(appDetails)
            }
            break
            case "tono": {
                if (!args[0]) return reply(`Enviar comando:\n${prefix+command} Título del tono\n\nEjemplo de uso:\n${prefix+command} BTS - Fire`)
                reply(lang.wait)
                let res = await fetch(`https://api.akuari.my.id/downloader/ringtone?judul=${encodeURIComponent(args[0])}`)
                if (!res.ok) throw await res.text()
                var result = await res.json()
                var ringtones = result.respon

                // Selecciona un tono de llamada al azar de la lista
                var randomIndex = Math.floor(Math.random() * ringtones.length)
                var randomRingtone = ringtones[randomIndex]

                // Descarga y envía el tono de llamada
                let audioRes = await fetch(randomRingtone.audio)
                if (!audioRes.ok) throw await audioRes.text()
            ...
            break
            case "guardar1": {
                if (!alpha.message.reply_to_message || !alpha.message.reply_to_message.document) return reply(`Por favor, responde a un mensaje que contenga el archivo que quieres guardar.`)
                let fileLink = await bot.telegram.getFileLink(alpha.message.reply_to_message.document.file_id)

                // Primera API
                let res1 = await fetch(`https://api.akuari.my.id/uploader/uploadfile?link=${encodeURIComponent(fileLink)}`)
                if (!res1.ok) throw await res1.text()
                var result1 = await res1.json()
                var fileLink1 = result1.respon

                reply(`Tu archivo ha sido guardado con la primera API. Puedes acceder a él en el siguiente enlace: ${fileLink1}`)
            }
            break
            case "guardar2": {
                if (!alpha.message.reply_to_message || !alpha.message.reply_to_message.document) return reply(`Por favor, responde a un mensaje que contenga el archivo que quieres guardar.`)
                ...
                // Segunda API
                let res2 = await fetch(`https://api.akuari.my.id/uploader/akuari?link=${encodeURIComponent(fileLink)}`)
                if (!res2.ok) throw await res2.text()
                var result2 = await res2.json()
                var fileLink2 = result2.result.url

                reply(`Tu archivo ha sido guardado con la segunda API. Puedes acceder a él en el siguiente enlace: ${fileLink2}`)
            }
            break
            case "guardar3": {
                if (!alpha.message.reply_to_message || !alpha.message.reply_to_message.photo) 
                    return reply(`Por favor, responde a un mensaje que contenga la imagen que quieres guardar.`)

                let fileLink = await bot.telegram.getFileLink(alpha.message.reply_to_message.photo.file_id)

                // Segunda API (ImgBB)
                let res2 = await axios.post(`https://api.imgbb.com/1/upload`, {
                    key: 'TU_CLAVE_API',
                    image: fileLink
                })

                if (!res2.data.success) 
                    throw new Error(res2.data.status_txt)

                var fileLink2 = res2.data.data.url

                reply(`Tu imagen ha sido guardada con la segunda API (ImgBB). Puedes acceder a ella en el siguiente enlace: ${fileLink2}`)
            }
            break;
            case "acortar": {
                if (!args[0]) return reply(`Enviar comando:\n${prefix+command} Enlace\n\nEjemplo de uso:\n${prefix+command} https://www.youtube.com/watch?v=PDEdwp04Tv8`)
                if (!isUrl(args[0])) return reply(`Enviar comando:\n${prefix+command} Enlace\n\nEjemplo de uso:\n${prefix+command} https://www.youtube.com/watch?v=PDEdwp04Tv8`)
                reply(lang.wait)
                let res = await fetch(`https://api.akuari.my.id/short/tinyurl?link=${encodeURIComponent(args[0])}`)
                if (!res.ok) throw await res.text()
                var result = await res.json()
                var shortUrl = result.hasil
                reply(`Aquí está tu enlace acortado: ${shortUrl}`)
            }
            break
            default:
        }
